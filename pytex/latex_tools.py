...
def table2csv(file_path, end_pattern=None, head=False, remove_math=True, save_name='latex_table', clean_pattern=None):
    """
    ...
    :param remove_math: bool, whether to remove math environment $ $
        ...
        if end_pattern in line:
            if head and not head_log:  # read head line
                head_list = _read_line_content(line.strip()[:-len(end_pattern)],
                                               remove_math=remove_math, clean_pattern=clean_pattern)
                head_log = True
            else:
...
def _read_line_content(line, remove_math=True, clean_pattern=None):
    content = line.split('&')
    content = [_clean_content(i, remove_math=remove_math, clean_pattern=clean_pattern) for i in content]
    return content


def _clean_content(content, remove_math=True, clean_pattern=None):
